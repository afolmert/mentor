...
def generate_buildno(path):
    ...
    if git_root_dir:
        try:
            ...
            # check if is dirty
            suffix = ''
            output = subprocess.Popen(['git', 'diff', 'head'], stdout=subprocess.PIPE).communicate()[0]
            if output.strip() != '':
                suffix += '-dirty'
            os.chdir(olddir)
            buildsha1 = headsha1[0:6] + suffix
        except:
            buildsha1 = '?'
...
def save_stamped_buildno():
    ...
    assert git_root_dir <> '', 'GIT root dir not found!'
    f = open(os.path.join(git_root_dir, 'src/build_stamp.py'), 'wt')
    ...
    f.write('buildno = "%s"' % generate_buildno(git_root_dir))
    f.close()

def delete_stamped_buildno():
    git_root_dir = get_git_root_dir(os.getcwd())
    assert git_root_dir <> '', 'GIT root dir not found!'
    try:
        os.remove(os.path.join(git_root_dir, 'src/build_stamp.py'))
        os.remove(os.path.join(git_root_dir, 'src/build_stamp.pyc'))
    except:
        pass
